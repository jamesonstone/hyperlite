...
It is regular UTF-8 text rendered with JetBrainsMono Nerd Font through the
shared application resolver and a system monospaced fallback, with no Markdown
presentation. Its darker inset surface follows the existing Selenized theme,
and its content never enters thread inference or attention.
...
`~/.local/share/hyperlite/notepad.txt` by default. On first use, Hyperlite adopts
the prior default `notepad.md` file without changing its content. The document
